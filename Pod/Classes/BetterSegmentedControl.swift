...
//
    ...
    @IBDesignable open class BetterSegmentedControl: UIControl {
        ...
        public required init?(coder aDecoder: NSCoder) {
            index = 0
            segments = Self.generateDefaultSegments()
...
//            let elementWidth = (width - totalInsetSize) / CGFloat(normalSegmentViewCount)
        ...
        private static func generateDefaultSegments() -> [LabelSegment] {
            ...
            let second = NSAttributedString(string: "Second")
            let third = NSAttributedString(string: "Third")

            return [.init(normalText: first, selectedText: first),
                    .init(normalText: second, selectedText: second),
                    .init(normalText: third, selectedText: third)]
        }
